...
const workExperiences = ['LikeMindedLab', 'Stingray', 'Dakis', 'OrangeTango', 'OneProvider'];
...
app.get('/works', async (req, res) => {
	...
	try {
        ...
        tmpDocuments = await collection.find({ ip: clientIp }, { projection: { _id: 0, ip: 0 } }).toArray();
...
app.put('/api/vote', async (req, res) => {
    ...
    if ( clientIp && workExperiences.includes(workExperience) ){
        ...
        try {
            ...
            const update = { $set: { ip: clientIp, workExperience: workExperience,  vote: (vote === 1 ? 1 : -1 ) }};
